...
class netflixExtractor(object):
	...
	def getSubtitleURL(self):
		...
		try:
			...
			print("Failed to fetch subtitles resource. Please ensure that subtitles are switched on by default for your account")
			pass
	...
	def loginNetflix(self):
		...
		netflixDriver.find_element_by_name(paths['usernameBox']).send_keys(username) 
		...
		netflixDriver.find_element_by_name(paths['passwordBox']).send_keys(password)
	...
	def getResources(self,driver):
		...
		while not Found:
			...
			self.resourceList = driver.execute_script("return window.performance.getEntries();")
			time.sleep(1)
			print(".",end="")
