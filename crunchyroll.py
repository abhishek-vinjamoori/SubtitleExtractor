...
class crunchyrollExtractor(object):
	...
	def decryptSubtitleData(self):
		...
		self.subtitleIV = self.bytes_to_intlist(base64.b64decode(self.subtitleIV.encode('utf-8')))
		...
		def obfuscate_key(key):
			...
			shaHash = self.bytes_to_intlist(sha1(prefix + str(num4).encode('ascii')).digest())
	...
	def aes_cbc_decrypt(self,key):
		...
		block_count = int(ceil(float(len(self.encryptedData)) / BLOCK_SIZE_BYTES))

		decrypted_data = []
		previous_cipher_block = self.subtitleIV
		for i in range(block_count):
			block = self.encryptedData[i * BLOCK_SIZE_BYTES: (i + 1) * BLOCK_SIZE_BYTES]
